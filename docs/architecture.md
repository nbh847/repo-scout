...
3. 根据仓库说明和技术信息生成中文摘要与中文说明。
4. 按 owner 和 name upsert 仓库记录。
5. 为本次抓取写入榜单快照。
6. 可选：把候选项目传入 AI 精选层。
7. 存储 AI 精选项目和入选理由。

首页项目卡片使用持久化的中文摘要。项目详情页同时展示中文说明和 GitHub 原始说明。当前未配置模型时使用可复现的本地分类模板生成中文内容，避免页面请求时重复翻译；后续配置模型后可在入库阶段提升摘要和说明质量。
...
规则评分负责可解释的筛选和排序，AI 只负责把结构化评分结果转成用户能理解的理由。模型供应商通过 curation 模块内的 OpenAI-compatible 调用封装，业务流程仍只依赖统一的精选生成入口；MVP 默认使用“规则评分 + 本地模板理由”运行，保证没有模型配置时也不阻塞核心流程。配置 OpenAI-compatible 模型环境变量后，再启用 AI 生成理由。

AI 模型配置默认规则：

- `REPO_SCOUT_OPENAI_BASE_URL` 指向 OpenAI-compatible API 根路径，例如包含 `/v1` 的 base URL。
- `REPO_SCOUT_OPENAI_API_KEY` 从环境变量读取，不写入代码、文档示例值或提交记录。
- `REPO_SCOUT_OPENAI_MODEL` 指定实际模型名称，并写入精选专题的 `model_name`。
- 缺少模型配置时，精选流程必须自动降级为本地模板理由，不报错中断。
- 模型调用失败或返回空理由时，单条项目理由自动回退到本地模板，不影响本次精选入库。
...
`POST /api/admin/curate` 复用同一套 MVP 权限策略。它从最新成功的 GitHub Trending run 读取候选项目，使用规则评分生成 `beginner-friendly-ai`、`ai-agent-projects`、`llm-tools` 和 `notable-developer-tools` 精选专题。缺少模型配置时，使用本地模板理由并将 `model_name` 写为 `local-template`；模型配置完整时，使用 OpenAI-compatible `chat/completions` 为每个入选项目生成一句精选理由。
