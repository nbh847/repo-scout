...
- 首页榜单支持按榜单排名、Stars 总量和新增 Stars 切换排序。
- 首页空结果状态支持一键清除筛选并返回全部项目。
...
- 修复专题页项目卡片中长仓库名导致最右卡片语言标签溢出边界的问题，给名字容器加 `min-w-0` 让其可收缩并正常换行。
...
- 2026-06-28：已通过 lint、typecheck、test:view-models 和 `next build`，验证专题页卡片名字容器加 `min-w-0` 后长仓库名不再把语言标签挤出卡片边界。
